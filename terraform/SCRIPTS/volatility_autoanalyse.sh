...
for dumpmem in `find $DMP_dir -type f | egrep '*\.dmp$|*\.vmem$' 2>/dev/null`; 
do 
  DMP_filename=`basename $dumpmem`
  ...
  do 
    ...
    result=`timeout 6s $VOLATILITY -f $dumpmem --profile=$profile --kdbg=$kdbg --dtb=$dtb --output=greptext filescan | wc -l`
